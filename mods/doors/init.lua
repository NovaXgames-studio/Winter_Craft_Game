...
function doors.register(name, def)
	...
	local def_a = table.copy(def)
	def_a.mesh = "door_a.obj"
	minetest.register_node(":" .. name .. "_a", def_a)

	local def_b = table.copy(def)
	def_b.mesh = "door_b.obj"
	minetest.register_node(":" .. name .. "_b", def_b)
...
function doors.register_fencegate(name, def)
	local fence = {
		description = def.description,
		drawtype = "mesh",
		tiles = {{name = def.texture, backface_culling = true}},
		paramtype = "light",
		paramtype2 = "facedir",
		sunlight_propagates = true,
	...
	fence_open.groups = table.copy(fence.groups)
	...
	local ok, err = pcall(minetest.register_node, ":" .. name .. "_closed", fence_closed)
	if not ok then
		minetest.log("warning", "[doors] fencegate mesh registration failed (closed): " .. tostring(err))
		local fallback_closed = {
			description = def.description,
			drawtype = "normal",
			tiles = {def.texture},
			paramtype = "light",
			...
			is_ground_content = false,
			drop = name .. "_closed",
			groups = table.copy(fence.groups),
			sounds = fence.sounds,
			on_rightclick = fence.on_rightclick,
		}
		minetest.register_node(":" .. name .. "_closed", fallback_closed)
	end

	ok, err = pcall(minetest.register_node, ":" .. name .. "_open", fence_open)
	if not ok then
		minetest.log("warning", "[doors] fencegate mesh registration failed (open): " .. tostring(err))
		local fallback_open = {
			description = def.description,
			drawtype = "normal",
			tiles = {def.texture},
			...
			is_ground_content = false,
			drop = name .. "_closed",
			groups = table.copy(fence_open.groups),
			sounds = fence.sounds,
			on_rightclick = fence.on_rightclick,
		}
		minetest.register_node(":" .. name .. "_open", fallback_open)
	end
